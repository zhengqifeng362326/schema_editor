...
export const queryNodesByComponentPath = (node: TreeNode, path: ComponentNameMatcher[]): TreeNode[] => {
  ...
  return matchComponent(node, path[0])
    ? node.children.reduce(
        (buf: TreeNode[], child: TreeNode) => buf.concat(queryNodesByComponentPath(child, path.slice(1))),
        [],
      )
    : [];
};
...
export const createEnsureTypeItemsNode = (type: string) => (node: TreeNode) => {
  const objectNode = node.children.find((child: TreeNode) => child?.props?.type === type);
  if (objectNode) {
    return objectNode;
  } else {
